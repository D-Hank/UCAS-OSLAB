...
//Notice: swap pages are ignored here!
void dealloc_ph_page(uintptr_t vaddr, PTE *pg_dir){
    unsigned long long vpn2 = ((unsigned long long)vaddr >> (NORMAL_PAGE_SHIFT + PPN_BITS + PPN_BITS)) & VPN_MASK;
    unsigned long long vpn1 = ((unsigned long long)vaddr >> (NORMAL_PAGE_SHIFT + PPN_BITS)) & VPN_MASK;
    unsigned long long vpn0 = ((unsigned long long)vaddr >> (NORMAL_PAGE_SHIFT)) & VPN_MASK;

    PTE *second = (PTE *)pa2kva(get_pa(pg_dir[vpn2]));
    PTE *third  = (PTE *)pa2kva(get_pa(second[vpn1]));
    uintptr_t phaddr = get_pa(third[vpn0]);
...
PTE *get_pte(uintptr_t vaddr, PTE *pg_dir){
    unsigned long long vpn2 = ((unsigned long long)vaddr >> (NORMAL_PAGE_SHIFT + PPN_BITS + PPN_BITS)) & VPN_MASK;
    unsigned long long vpn1 = ((unsigned long long)vaddr >> (NORMAL_PAGE_SHIFT + PPN_BITS)) & VPN_MASK;
    unsigned long long vpn0 = ((unsigned long long)vaddr >> (NORMAL_PAGE_SHIFT)) & VPN_MASK;

    if((pg_dir[vpn2] & _PAGE_PRESENT) == 0){
        set_pfn(&pg_dir[vpn2], kva2pa(alloc_pgtab()) >> NORMAL_PAGE_SHIFT);
        set_attribute(&pg_dir[vpn2], _PAGE_PRESENT);
        clear_pgdir(pa2kva(get_pa(pg_dir[vpn2])));
    }

    PTE *second = (PTE *)pa2kva(get_pa(pg_dir[vpn2]));
    if((second[vpn1] & _PAGE_PRESENT) == 0){
        set_pfn(&second[vpn1], kva2pa(alloc_pgtab()) >> NORMAL_PAGE_SHIFT);
        set_attribute(&second[vpn1], _PAGE_PRESENT);
        clear_pgdir(pa2kva(get_pa(second[vpn1])));
    }
...
uintptr_t shm_page_get(int key)
{
    ...
    for(i = 0; i < SHM_PAGE_NUM; i++){
        if(shm_page_array[i].ref > 0 && shm_page_array[i].key == key){
            shm_page_array[i].ref++;
            first = 0;
            goto good;
        }
    }
    ...
    if(found == 0){
        return 0;// shm_page not enough
    }

    int mycpu_id = get_current_cpu_id();
    shm_page_array[i].key = key;
    shm_page_array[i].ref = 1;
    //shm_page_array[i].phaddr = kva2pa(alloc_ph_page(current_running[mycpu_id]->pg_dir));
    first = 1;

good:
{
    /*uintptr_t ret = SHM_IDX_TO_ADDR(i);
    int mycpu_id = get_current_cpu_id();
    alloc_page_helper(ret, current_running[mycpu_id]->pg_dir);
    return ret;*/

    uintptr_t ret = USER_SHM_START;
    for(;ret < USER_SHM_END; ret += 0x1000){
        PTE *pte = get_pte(ret, current_running[mycpu_id]->pg_dir);
        if(((*pte) & _PAGE_PRESENT) == 0){//va not used
            if(first == 1){
                uintptr_t kvaddr = alloc_ph_page(pte);
                uintptr_t phaddr = kva2pa(kvaddr);
                shm_page_array[i].phaddr = phaddr;
                kmemset(kvaddr, 0, NORMAL_PAGE_SIZE);
            }
            set_pfn(pte, (shm_page_array[i].phaddr) >> NORMAL_PAGE_SHIFT);
            set_attribute(pte, _PAGE_PRESENT | _PAGE_READ | _PAGE_WRITE | _PAGE_EXEC | _PAGE_ACCESSED | _PAGE_DIRTY | _PAGE_USER);
...
{
    // TODO(c-core):
    int index = SHM_ADDR_TO_IDX(addr);
    shm_page_array[index].ref--;

    if(shm_page_array[index].ref == 0){
        int mycpu_id = get_current_cpu_id();
        dealloc_ph_page(addr, current_running[mycpu_id]->pg_dir);
    }
}
