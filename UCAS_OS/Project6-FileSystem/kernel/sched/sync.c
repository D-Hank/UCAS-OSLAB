...
int do_mbox_open(char *name){
    ...
    for(i=0;name[i]!=0;i++){
        mbox_array[mbox_num].name[i]=name[i];
    }
    mbox_array[mbox_num].name[i]=0;
...
void return_v(reg_t a0){
    int mycpu_id=get_current_cpu_id();
    regs_context_t * cont=(regs_context_t *)(current_running[mycpu_id]->kernel_stack_base - sizeof(regs_context_t));
    reg_t * cont_r=cont->regs;
    cont_r[10]=a0;
}

int do_mbox_send(int id, void *msg, int msg_length){
    int mycpu_id=get_current_cpu_id();
    if(mbox_array[id].tail+msg_length>=MBOX_BUFFER){
        ...
        kmemcpy(&(mbox_array[id].buffer[mbox_array[id].tail]),msg,msg_length);
...
int do_mbox_recv(int id, void *msg, int msg_length){
    int mycpu_id=get_current_cpu_id();
    if(msg_length>mbox_array[id].tail){
        ...
        mbox_array[id].tail-=msg_length;
        kmemcpy(msg,&(mbox_array[id].buffer[mbox_array[id].tail]),msg_length);
        /*if(mbox_array[id].head+msg_length==mbox_array[id].tail){
            mbox_array[id].head=0;
            mbox_array[id].tail=0;
        }else{
