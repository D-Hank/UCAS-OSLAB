...
int do_kill(pid_t pid){
    ...
    unmap_io(pcb[i].pg_dir);

    //----------------------------NOTICE--------------------------
    // We have NOT clear the net_send/recv_queue yet!!!
    //------------------------------------------------------------
...
pid_t do_exec(const char* file_name, int argc, char* argv[], spawn_mode_t mode){
    ...
    ptr_t kva = get_kva_of(USER_STACK_ADDR - STACK_VICTIM, pg_dir);//dest addr
    //when we call get_kva_of, it doesn't need to return a 4K align addr
