...
void *shmpageget(int key){
    return (void *)invoke_syscall(SYSCALL_SHMPAGEGET, key, IGNORE, IGNORE,IGNORE);
}
...
int sys_fopen(char *file_name, access_t access){
    return invoke_syscall(SYSCALL_FOPEN, file_name, access, IGNORE, IGNORE);
}
...
int sys_mkfs(){
    return invoke_syscall(SYSCALL_MK_FS, IGNORE, IGNORE, IGNORE, IGNORE);
}

void sys_statfs(){
    invoke_syscall(SYSCALL_STAT_FS, IGNORE, IGNORE, IGNORE, IGNORE);
}

int sys_mkdir(char *dir_name){
    return invoke_syscall(SYSCALL_MK_DIR, dir_name, IGNORE, IGNORE, IGNORE);
}

int sys_rmdir(char *dir_name){
    return invoke_syscall(SYSCALL_RM_DIR, dir_name, IGNORE, IGNORE, IGNORE);
}

void sys_ls(int argc, char *argv[]){
    invoke_syscall(SYSCALL_LIST_DIR, argc, argv, IGNORE, IGNORE);
}

int sys_cd(char *dir_name){
    return invoke_syscall(SYSCALL_CHG_DIR, dir_name, IGNORE, IGNORE, IGNORE);
}

int sys_mknod(char *file_name){
    return invoke_syscall(SYSCALL_MK_NOD, file_name, IGNORE, IGNORE, IGNORE);
}

int sys_cat(char *file_name){
    return invoke_syscall(SYSCALL_CAT, file_name, IGNORE, IGNORE, IGNORE);
}

int sys_link(char *src, char *dest){
    return invoke_syscall(SYSCALL_LINK, src, dest, IGNORE, IGNORE);
}

int sys_unlink(char *file_name){
    return invoke_syscall(SYSCALL_UNLINK, file_name, IGNORE, IGNORE, IGNORE);
}

int sys_lseek(int fd, int offset, int whence, char rw){
    return invoke_syscall(SYSCALL_LSEEK, fd, offset, whence, rw);
}

void sys_remake(){
    invoke_syscall(SYSCALL_REMAKE, IGNORE, IGNORE, IGNORE, IGNORE);
}
