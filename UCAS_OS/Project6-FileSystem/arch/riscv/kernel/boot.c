...
uintptr_t alloc_page()//allocate a page in real mode
{
    static uintptr_t pg_base = PGDIR_PA;//initialize once
    pg_base += 0x1000;
    return pg_base;
}
...
void setup_vm()
{
    ...
    // collect these available spaces
    ...
    // map boot address
    // this boot program uses direct mapping virtual address
    // configure 0x50000000 ~ 0x50400000, each time by 2MB
